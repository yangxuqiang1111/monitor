 ...
 */
@Component
public class UDPDelayServer implements IBaseServer,ApplicationContextAware {
    private Logger log = Logger.getLogger(UDPDelayServer.class);
    ...
    public void start(final int port) {
        ...
        b.bind(port).addListener(new ChannelFutureListener() {
            public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    log.info("UDPDelayServer 启动成功, listen on [" + port + "]");
                } else {
                    log.info("UDPDelayServer 启动失败, listen on [" + port + "]");
                }
            }
        });
